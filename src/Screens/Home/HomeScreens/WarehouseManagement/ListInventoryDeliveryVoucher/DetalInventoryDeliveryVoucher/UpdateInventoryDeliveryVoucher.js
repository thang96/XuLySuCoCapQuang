...
import {GetInventoryDeliveryVoucherByIDAPI} from '../../../../../../Api/Home/StableWarehouseAPI/StableWarehouseAPI';
const UpdateInventoryDeliveryVoucher = props => {
    ...
    await GetInventoryDeliveryVoucherByIDAPI(token, id)
      .then(res => {
        if (res?.status == 200 && res?.data?.success == true) {
          ...
          setDeliveryTime(res?.data?.data?.delivery_time);
          setFormatsDate(res?.data?.data?.delivery_time);
          setLoading(false);
  ...
  const updateVoucher = async () => {
    ...
    await UpdateAInventoryDeliveryVoucherAPI(
      token,
      id,
      stable_warehouse_id,
      deliveryTime,
      ...
      .then(res => {
        if (res?.status == 200 && res?.data?.success == true) {
          Alert.alert('Sửa phiếu xuất kho', 'Sửa phiếu xuất kho thành công');
          navigation.navigate('ListInventoryDeliveryVoucher');
        } else if (res?.status == 200 && res?.data?.success == false) {
          Alert.alert('Sửa phiếu xuất kho', 'Không thể sửa phiếu xuất kho');
        }
      })
      .catch(function (error) {
        // console.log(error);
        Alert.alert('Sửa phiếu xuất kho', 'Sửa phiếu xuất kho thất bại');
      });
  };
...
export default UpdateInventoryDeliveryVoucher;
