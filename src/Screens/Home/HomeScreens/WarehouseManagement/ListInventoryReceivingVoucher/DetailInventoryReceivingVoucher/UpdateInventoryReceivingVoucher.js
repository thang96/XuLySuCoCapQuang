...
import {GetInventoryReceivingVoucherByIDAPI} from '../../../../../../Api/Home/StableWarehouseAPI/StableWarehouseAPI';
const UpdateInventoryReceivingVoucher = props => {
  ...
  const getResult = async () => {
    let id = route?.params;
    await GetInventoryReceivingVoucherByIDAPI(token, id)
      .then(res => {
        if (res?.status == 200 && res?.data?.success == true) {
          setIdUpdate(res?.data?.data?.id);
          setStableWarehouseId(res?.data?.data?.stable_warehouse);
          setReceiveTime(res?.data?.data?.receive_time);
          setFormatsDate(res?.data?.data?.receive_time);
          setLoading(false);
  ...
  const updateVoucher = async () => {
    ...
    await UpdateAInventoryReceivingVoucherAPI(
      token,
      id,
      stable_warehouse_id,
      receiveTime,
      ...
      .then(res => {
        if (res?.status == 200 && res?.data?.success == true) {
          Alert.alert('Sửa phiếu nhập kho', 'Sửa phiếu nhập kho thành công');
          navigation.navigate('ListInventoryReceivingVoucher');
        } else if (res?.status == 200 && res?.data?.success == false) {
          Alert.alert('Sửa phiếu nhập kho', 'Không thể sửa phiếu nhập kho');
        }
      })
      .catch(function (error) {
        console.log(error);
...
export default UpdateInventoryReceivingVoucher;
